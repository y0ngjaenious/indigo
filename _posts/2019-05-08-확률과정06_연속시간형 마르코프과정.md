...
- 아래 3가지를 정의
  ...
  3. $\bold{P}(t)=[P_{ij}(t)]=\begin{bmatrix} P_{00}(t) &P_{01}(t)&...\\P_{10}(t)&P_{11}(t)&...\\...&...&...\end{bmatrix}$

- 마르코프과정의 성질

  1. $P(X(t)=j)=\Sigma_iP(X(t)=j,X(0)=i)=\sum_ip_iP_{ij}(t)$
  2. $\bold{P}(t+s)=\bold{P}(t)\bold{P}(s)=\bold{P}(s)\bold{P}(t)$
...
- 마르코프 확률과정에서 상태 i에 머무는 시간 $T_i$는 지수분포를 한다(모수는 $v_i$라 가정). 충분히 작은 시간 h 내에 상태가 2번 이상 변할 확률은 o(h)가 되므로 아래 식이 성립한다
  ...
  - $lim_{h\to0}\frac{1-P_{ii}(h)}{h}=v_i$
...
- 행렬로 표현
  ...
  - $\bold{P}(h+t)=\bold{P}(h)*\bold{P}(t)$

  - 양변에서 $\bold{P}(t)$를 빼고 h로 나눈 뒤 $h\to0$를 취하면 아래 식이 성립한다

    - $\bold{Q}(전이율행렬)=[q_{ij}]$

      ​                         $=lim_{h\to0}\frac{\bold{P}(h)-I}{h}$

      - $q_{ii}=-v_i,\quad q_{ij}=v_iP_{ij}\quad(i\ne j)$

    - $\frac{d}{dt}\bold{P}(t)=\bold{Q}*\bold{P}(t)$
...
- **콜모고로프의 전진방정식**
  ...
  - $\frac{d}{dt}\bold{P}(t)=\bold{P}(t)*\bold{Q}$
...
- {$X_t|t\ge0$}가 기약마르코프과정이면 $\pi_j=lim_{t\to\infty}P_{ij}(t)$ 존재
